...
def extract_experience(resume_text):
    '''
    Helper function to extract experience from resume
    ...
    filtered_sentence = [
            w for w in word_tokens if w not in stop_words and wordnet_lemmatizer.lemmatize(w) not in stop_words
        ]
    ...
    # Search for year
    x = [
        (i, i.lower().index(re.search(cs.YEAR, i.lower()).group()))
        for i in test
        if re.search(cs.YEAR, i.lower())
    ]

    return x


def robust_extract_entity_sections(text):
    '''
    Helper function to extract all the raw text from sections of a resume.
    It's more robust than the original functions.
    :param text: Raw text of resume
    :return: dictionary of entities
    '''
    entities = {}
    ...
    # Combine all known section headers for matching
    all_section_headers = set([h.lower() for h in cs.RESUME_SECTIONS_GRAD] + [h.lower() for h in cs.RESUME_SECTIONS_PROFESSIONAL])
    
    for line in text.split('\n'):
        line_clean = line.strip().lower()

        if line_clean in all_section_headers:
        ...
        if current_section and line.strip():
            entities[current_section].append(line.strip())
